...
export function useNotificationSocket() {
    function connect(userId: number, onNotification: (notification: NotificationVO) => void): void {
        ...
        client = new Client({
                ...
                subscription = client!.subscribe(
                    ...
                    (msg: IMessage) => {
                        try {
                            ...
                            if (frame.event === "notification.new" && frame.data) {
            ...
            onStompError: (frame) => {
                console.error("[STOMP-Notification] 错误:", frame.headers["message"]);
            },
        });
        client.activate();
    }
