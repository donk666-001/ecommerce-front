  ...
  <div class="admin-app">
    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-account-wrap" ref="accountMenuRef">
          ...
          <div v-if="accountMenuOpen" class="account-menu">
            <div class="account-profile">
              <div class="admin-avatar account-avatar font-serif">
                <img v-if="avatarSrc" :src="avatarSrc" alt="头像" class="admin-avatar-img" />
            ...
            <button type="button" class="account-menu-item" @click="goSettings">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .34 1.87l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06A1.7 1.7 0 0 0 15 19.4a1.7 1.7 0 0 0-1 .6 1.7 1.7 0 0 0-.4 1.1V21a2 2 0 1 1-4 0v-.09A1.7 1.7 0 0 0 8.6 19.4a1.7 1.7 0 0 0-1.87.34l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.7 1.7 0 0 0 4.6 15a1.7 1.7 0 0 0-.6-1 1.7 1.7 0 0 0-1.1-.4H3a2 2 0 1 1 0-4h.09A1.7 1.7 0 0 0 4.6 8.6a1.7 1.7 0 0 0-.34-1.87l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.7 1.7 0 0 0 9 4.6a1.7 1.7 0 0 0 1-.6 1.7 1.7 0 0 0 .4-1.1V3a2 2 0 1 1 4 0v.09A1.7 1.7 0 0 0 15.4 4.6a1.7 1.7 0 0 0 1.87-.34l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.7 1.7 0 0 0 19.4 9c.2.38.6.6 1 .6h.6a2 2 0 1 1 0 4h-.09a1.7 1.7 0 0 0-1.51 1.4z"/></svg>
              <span>设置</span>
            </button>
        ...
        >
          ...
          <svg class="account-gear" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .34 1.87l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06A1.7 1.7 0 0 0 15 19.4a1.7 1.7 0 0 0-1 .6 1.7 1.7 0 0 0-.4 1.1V21a2 2 0 1 1-4 0v-.09A1.7 1.7 0 0 0 8.6 19.4a1.7 1.7 0 0 0-1.87.34l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.7 1.7 0 0 0 4.6 15a1.7 1.7 0 0 0-.6-1 1.7 1.7 0 0 0-1.1-.4H3a2 2 0 1 1 0-4h.09A1.7 1.7 0 0 0 4.6 8.6a1.7 1.7 0 0 0-.34-1.87l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.7 1.7 0 0 0 9 4.6a1.7 1.7 0 0 0 1-.6 1.7 1.7 0 0 0 .4-1.1V3a2 2 0 1 1 4 0v.09A1.7 1.7 0 0 0 15.4 4.6a1.7 1.7 0 0 0 1.87-.34l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.7 1.7 0 0 0 19.4 9c.2.38.6.6 1 .6h.6a2 2 0 1 1 0 4h-.09a1.7 1.7 0 0 0-1.51 1.4z"/></svg>
        </button>
      </div>
    </aside>
    ...
    <Teleport to="body">
      <Transition name="settings-modal">
        <div v-if="settingsVisible" class="settings-mask" @click.self="closeSettingsModal">
          <div class="settings-modal">
            <div class="settings-header">
              ...
              <button type="button" class="settings-close" @click="closeSettingsModal">×</button>
            </div>

            <div class="settings-body">
              <div class="settings-avatar-row">
                <button type="button" class="settings-avatar" @click="triggerAvatarUpload">
                  <img v-if="settingsAvatarSrc" :src="settingsAvatarSrc" alt="头像" />
                  <span v-else class="font-serif">{{ displayInitial }}</span>
                  <em>更换</em>
                </button>
              ...
              <label class="settings-field">
                <span>昵称</span>
                <input v-model.trim="settingsForm.nickname" type="text" placeholder="请输入昵称" />
              </label>

              <label class="settings-field">
                <span>邮箱</span>
                <input v-model.trim="settingsForm.email" type="email" placeholder="example@mail.com" />
              </label>

              <label class="settings-field">
                <span>手机号</span>
                <input v-model.trim="settingsForm.phone" type="tel" placeholder="请输入手机号" />
              </label>
            ...
            <div class="settings-footer">
              <button type="button" class="settings-btn" :disabled="savingSettings" @click="closeSettingsModal">取消</button>
              <button type="button" class="settings-btn primary" :disabled="savingSettings" @click="saveSettings">
...
const settingsAvatarSrc = computed(() => avatarPreview.value || avatarSrc.value)
...
function openSettingsModal() {
  settingsForm.nickname = userStore.G_LoginInfo.nickName || userStore.G_LoginInfo.account || ''
  settingsForm.email = userStore.G_UserInfo.email || userStore.G_LoginInfo.email || ''
...
async function saveSettings() {
  ...
  try {
    ...
    if (!result.success) {
      showSettingsMessage(result.message || '保存失败，请稍后重试', 'error')
      return
    }
...
.sidebar-account.open .account-gear { transform:rotate(32deg); }
.account-menu {
  position:absolute;
  left:12px;
  right:12px;
  ...
  border-radius:14px;
  box-shadow:0 18px 38px rgba(60,50,30,0.16), 0 2px 8px rgba(60,50,30,0.08);
  padding:8px;
}
.account-profile {
  display:flex;
...
.settings-modal {
  ...
  box-shadow:0 28px 70px rgba(44,54,57,0.26), 0 8px 22px rgba(60,50,30,0.12);
  display:flex;
...
.settings-field select {
  appearance:none;
  background-image:linear-gradient(45deg, transparent 50%, #6B7C7A 50%), linear-gradient(135deg, #6B7C7A 50%, transparent 50%);
...
@media (max-width: 768px) {
  ...
  .settings-mask { padding:16px; }
  .settings-header,
  .settings-body,
  .settings-footer { padding-left:18px; padding-right:18px; }
