      ...
      >
        ...
        <div class="order-foot">
          <span class="order-amount">
            共<strong>{{ o.items.reduce((s, it) => s + it.qty, 0) }}</strong>
            件商品&emsp;合计<strong>¥{{ o.amount.toFixed(2) }}</strong>
          </span>
          <div class="order-actions">
            <template v-if="o.status === 'unpaid'">
              <button class="btn btn-outline" @click="$emit('cancelOrder', o.no)">
                取消订单
              </button>
              <button class="btn btn-cinnabar" @click="$emit('payOrder', o.no)">
                立即支付
              </button>
            </template>
            <template v-else-if="o.status === 'topay'">
              <button class="btn btn-outline" @click="$emit('openRefundModal', o.no)">
                申请退款
              </button>
              <button class="btn btn-outline" @click="$emit('remindShip', o.no)">
                提醒发货
              </button>
              <button class="btn btn-outline" @click="$emit('reBuy', o.no)">
                再次购买
              </button>
            </template>
            <template v-else-if="o.status === 'shipped'">
              ...
              <template v-if="o.refundStatus === undefined">
                ...
                <button class="btn btn-outline" @click="$emit('openLogistics', o.no)">
                  查看物流
                </button>
                <button class="btn btn-jade" @click="$emit('confirmReceipt', o.no)">
                  确认收货
                </button>
                <div class="more-wrap" @click.stop>
                  <button class="btn btn-outline" @click="$emit('toggleMore', idx)">
                    更多 ▾
                  </button>
                  <div
                    class="more-menu"
                  ...
                  >
                    ...
                    <div
                      class="more-menu-item"
                      @click="handleExtendReceiptAndClose(o.no)"
                    >
                      延长收货
                    </div>
                  </div>
              ...
              <template v-else-if="[0, 1, 2].includes(o.refundStatus)">
                <!-- 退款申请中、审核通过、退款中 -->
                <div class="refund-status-tip">
                  <span class="tip-icon">⏳</span>
                  <span class="tip-text">
                    {{ getRefundStatusText(o.refundStatus) }}，客服将在 24h 内处理
                  </span>
                </div>
                <button class="btn btn-outline" @click="$emit('openLogistics', o.no)">
                  查看物流
              ...
              <template v-else-if="o.refundStatus === 3">
                ...
                <div class="refund-completed-tip">
                  <span class="tip-icon">✅</span>
                  <span class="tip-text">退款已完成，款项将原路返回</span>
                </div>
                <button class="btn btn-outline" @click="$emit('reBuy', o.no)">
                  再次购买
                </button>
              ...
              <template v-else-if="o.refundStatus === 4">
                ...
                <div class="refund-rejected-tip">
                  ...
                  <div class="tip-content">
                    <span class="tip-text">退款申请已被拒绝</span>
                    <span v-if="o.auditRemark" class="tip-reason">
                      原因：{{ o.auditRemark }}
                    </span>
                  </div>
                </div>
            ...
            <template v-else>
              ...
              <button class="btn btn-outline" @click="$emit('reviewOrder', o.no)">
                评价
              </button>
              <button class="btn btn-outline" @click="$emit('reBuy', o.no)">
                再次拼单
