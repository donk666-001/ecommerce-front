...
<template>
  <section ref="sectionEl" class="ai-butler glass-section" id="ai-butler-section">
    ...
    <div class="container">
      <div class="ai-inner">
        <!-- 左侧 -->
        <div class="ai-left">
          <div class="section-tag" data-animate="from-left">✦ AI 管家</div>
          <h2 class="section-title" data-animate="from-left" style="--delay:80ms">
            你的专属<br />东方健康顾问
          </h2>
          <p class="section-sub" data-animate="from-left" style="--delay:160ms">
            基于中医体质理论 × 节气时令 × 大语言模型，随时为你解答健康问题、生成个性方案。
          </p>
          <ul class="ai-feats">
            <li v-for="(f, i) in feats" :key="f.name"
                class="ai-feat"
        ...
        <!-- 右侧聊天窗口 -->
        <div class="ai-right" data-animate="from-right" style="--delay:200ms">
          <div class="chat-window">
            <div class="chat-header">
              <div class="chat-avatar">🤖</div>
              <div>
                <div class="chat-name">颐养 AI 管家</div>
                <div class="chat-status"><span class="chat-dot" />在线服务中</div>
              </div>
              <div class="chat-badge">由 DeepSeek 驱动</div>
            </div>
            <div class="chat-body">
              <div class="chat-sys">今天是芒种节气 🌾 已为你生成专属建议</div>
              <div v-for="msg in messages" :key="msg.id"
                   class="chat-row"
                   :class="[msg.role, { show: msg.visible }]">
                <div v-if="msg.type === 'bubble'" class="chat-bubble" :class="msg.role" v-html="msg.html" />
                <div v-else class="chat-referral">
                  <div class="referral-left">
                    <div class="referral-icon">👩‍⚕️</div>
                    <div>
                      <div class="referral-name">张明慧 · 主任中医师</div>
                      <div class="referral-org">北京协和医院 · 气虚调理专家</div>
                    </div>
                  </div>
                  <button class="referral-btn">立即问诊</button>
                </div>
              </div>
            </div>
            <div class="chat-input-bar">
              <input class="chat-input" type="text" placeholder="向 AI 管家提问…" readonly />
...
// 功能特性列表
const feats = [
  { icon: '🌿', name: '节气健康日历', desc: '每日推送当令饮食、作息与穴位建议' },
  { icon: '🧬', name: '体质辨识分析', desc: '问答式评估，生成九大体质专属方案' },
  { icon: '💬', name: '实时健康问答', desc: '随时提问，中西医知识融合解答' },
  { icon: '👩‍⚕️', name: '智能转诊推荐', desc: '识别需专业就诊情况，一键连接名医' },
]
...
.chat-avatar {
  ...
  border-radius: 12px;
  flex-shrink: 0;
  background: linear-gradient(135deg, rgba(93, 112, 82, 0.7), rgba(70, 90, 60, 0.9));
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 18px;
...
.referral-icon {
  ...
  border-radius: 10px;
  flex-shrink: 0;
  background: linear-gradient(135deg, rgba(93, 112, 82, 0.6), rgba(193, 140, 93, 0.6));
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 16px;
