...
<template>
  <section class="hero">
    ...
    <div class="hero-left">
      ...
      <div class="hero-btns">
        <button class="hero-btn-p" @click="$emit('go-login')">🌿 开启养生之旅</button>
...
const cards = [
  { icon: '🌿', name: '节气养生', desc: '随二十四节气变化，调整饮食起居，顺时而为', delay: '1.1s' },
  { icon: '🌙', name: '睡眠作息', desc: '追踪记录，优化你的自然睡眠节律', delay: '1.2s' },
  { icon: '🍵', name: '中医智慧', desc: '体质辨识，古法今用，调和阴阳平衡', delay: '1.3s' },
  { icon: '🤖', name: 'AI 管家', desc: '专属健康顾问，智能分析每日状态', delay: '1.4s' },
  { icon: '👩‍⚕️', name: '名医健康圈 · 在线问诊', desc: '50+ 三甲医院认证专家，随时在线，专业守护', wide: true, delay: '1.5s' },
]
...
.hero {
  ...
  &::after {
    ...
    right: -48px;
    height: 180px;
    background: linear-gradient(to bottom, transparent 0%, rgba(10, 16, 8, 0.15) 100%);
    pointer-events: none;
  }
}
...
.fg-card {
  ...
  overflow: hidden;
  cursor: default;
  transition: transform 0.3s cubic-bezier(0.34, 1.2, 0.64, 1), box-shadow 0.3s, background 0.3s;
  background: rgba(255, 255, 255, 0.11);
  backdrop-filter: blur(18px) saturate(1.45);
  -webkit-backdrop-filter: blur(18px) saturate(1.45);
  border: 1px solid rgba(255, 255, 255, 0.2);
  box-shadow: 0 6px 24px rgba(0, 0, 0, 0.18), inset 0 1px 0 rgba(255, 255, 255, 0.32);
  opacity: 0;
  ...
  &:hover {
    ...
    box-shadow: 0 18px 44px rgba(0, 0, 0, 0.26), inset 0 1px 0 rgba(255, 255, 255, 0.42);
...
.fg-icon { font-size: 28px; line-height: 1; filter: drop-shadow(0 2px 6px rgba(0, 0, 0, 0.22)); }
.fg-name { font-size: 14px; font-weight: 700; color: rgba(255, 255, 255, 0.96); margin-bottom: 5px; }
.fg-desc { font-size: 12px; color: rgba(255, 255, 255, 0.6); line-height: 1.6; }
...
@keyframes scrollLine {
  ...
  50%       { transform: scaleY(0.6) translateY(6px); opacity: 0.4; }
}
</style>
