<template>
  <div class="review-panel">
    ...
    <div class="toolbar-card">
      ...
      <div class="filter-center">
          ...
          <div class="custom-select" :class="{ open: roleDropOpen }" ref="roleDropRef">
            <button type="button" class="cs-trigger" @click="roleDropOpen = !roleDropOpen">
              <span :class="['cs-text', { placeholder: !filterRole }]">
                {{ filterRole ? roleLabel(filterRole) : '全部角色' }}
              </span>
            ...
            <div v-if="roleDropOpen" class="cs-menu">
              <div v-for="opt in roleOptions" :key="opt.value"
                class="cs-option" :class="{ active: filterRole === opt.value }"
                @click="pickRole(opt.value)">{{ opt.label }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="filter-group">
          ...
          <div class="sort-btns">
            <button type="button" class="sort-btn" :class="{ active: sortOrder === 'desc' }" @click="sortOrder='desc';currentPage=1;loadList()">最新↓</button>
            <button type="button" class="sort-btn" :class="{ active: sortOrder === 'asc' }" @click="sortOrder='asc';currentPage=1;loadList()">最早↑</button>
          </div>
        </div>
      ...
      <div class="search-input">
        ...
        <input v-model="keyword" type="text" placeholder="搜索姓名 / 用户 ID" @keyup.enter="currentPage=1;loadList()" />
        <button
          type="button"
          class="search-clear"
          ...
          @click="clearSearch"
        >×</button>
        <button type="button" class="search-btn" @click="currentPage=1;loadList()">查询</button>
      </div>
    </div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="app in applications" :key="app.id">
            ...
            <td style="text-align:center"><span class="tag tag-jade">{{ roleLabel(app.roleType) }}</span></td>
            <td><div class="bio-text">{{ truncateBio(app.bio) }}</div></td>
            <td style="text-align:center">
              <span class="file-count" style="cursor:pointer" @click="openDetail(app)">
                📄 附件*{{ app.attachments?.length ?? 0 }}
              </span>
            </td>
            <td class="cell-date">{{ formatDate(app.createdAt) }}</td>
            <td style="text-align:center"><span class="tag" :class="statusPillClass(app.status)">{{ statusLabel(app.status) }}</span></td>
            <td>
              <div class="btn-group">
                <button class="btn btn-ghost" @click="openDetail(app)">详情</button>
                <template v-if="app.status === 'SUBMITTED' || app.status === 'REVIEWING'">
                  <button class="btn btn-jade" @click="askApprove(app.id)">✓ 通过</button>
    ...
    <!-- ── Detail modal ── -->
    <div v-if="detailVisible" class="modal-mask show" @click.self="detailVisible = false">
      <div class="modal modal-detail">
          ...
          <template v-else-if="detail">
            <div class="detail-summary">
              ...
              <div class="detail-meta">
                <div class="detail-name">{{ detail.realName ?? '—' }}</div>
                <div class="detail-sub">
                  <span>用户 ID：{{ formatUserId(detail.userId) }}</span>
                  <span class="tag tag-jade" style="margin-left:8px">{{ roleLabel(detail.roleType) }}</span>
                  <span class="tag" :class="statusPillClass(detail.status)" style="margin-left:6px">{{ statusLabel(detail.status) }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="detail-info-row">
              <div class="detail-info-item">
                ...
                <span class="detail-info-value">{{ detail.email ?? '未填写' }}</span>
              </div>
              <div class="detail-info-item">
                <span class="detail-info-label">提交时间</span>
                <span class="detail-info-value">{{ formatDate(detail.createdAt) }}</span>
              </div>
            </div>
            ...
            <div class="detail-section">
              ...
              <div v-if="!detail.attachments?.length" style="color:var(--ink-muted);font-size:13px">无附件</div>
              <div v-else class="attachment-grid">
                <div v-for="att in detail.attachments" :key="att.docType" class="att-item">
                  ...
                  <img
                    ...
                    :alt="att.fileName"
                    class="att-img"
                    @click="openLightbox(`${MINIO_BASE}/${att.url}`, att.fileName)"
        ...
        <div v-if="detail && ['SUBMITTED','REVIEWING'].includes(detail.status)" class="modal-footer">
          <button class="modal-btn" @click="detailVisible = false">取消</button>
          <button class="modal-btn primary-cinnabar" @click="showRejectDialogFromDetail">✕ 驳回</button>
          <button class="modal-btn primary-jade" @click="askApproveFromDetail">✓ 通过</button>
        </div>
    ...
    <div v-if="approveConfirmVisible" class="modal-mask show" @click.self="approveConfirmVisible = false">
      <div class="modal" style="max-width:420px">
        <div class="modal-header">
          <h3 class="font-serif">确认审核通过</h3>
          <button class="modal-close" @click="approveConfirmVisible = false">×</button>
        </div>
        <div class="modal-body">
        ...
        <div class="modal-footer">
          <button class="modal-btn" @click="approveConfirmVisible = false">取消</button>
          <button class="modal-btn primary-jade" :disabled="approving" @click="confirmApprove">
            {{ approving ? '处理中…' : '✓ 确认通过' }}
          </button>
        </div>
      </div>
    ...
    <div v-if="rejectDialogVisible" class="modal-mask show" @click.self="rejectDialogVisible = false">
      <div class="modal">
        ...
        <div class="modal-footer">
          ...
          <button class="modal-btn primary-cinnabar" :disabled="rejecting" @click="confirmReject">
            {{ rejecting ? '处理中…' : '确认驳回' }}
          </button>
        </div>
      </div>
    ...
    <!-- ── Lightbox ── -->
    <div v-if="lightboxVisible" class="lightbox-mask" @click="lightboxVisible = false">
      <button class="lightbox-close" @click="lightboxVisible = false">×</button>
...
function showToast(type: 'success' | 'error', message: string) {
  ...
  toastTimer = window.setTimeout(() => { toast.visible = false }, 4500)
}
...
function onKeydown(e: KeyboardEvent) {
  if (e.key === 'Escape') {
    ...
    if (detailVisible.value)        { detailVisible.value = false; return }
    if (approveConfirmVisible.value){ approveConfirmVisible.value = false; return }
    if (rejectDialogVisible.value)  { rejectDialogVisible.value = false }
  }
}
...
async function confirmApprove() {
  ...
  try {
    const res = await ApiExpert.adminReviewApplication(approveTargetId, 'APPROVE')
    ensureSuccess(res)
...
async function confirmReject() {
  if (!rejectReason.value.trim()) { showToast('error', '请填写驳回原因'); return }
...
function statusPillClass(s: string) { return ({ SUBMITTED:'tag-gold', REVIEWING:'tag-gold', APPROVED:'tag-jade', SIGNED:'tag-jade', ACTIVATED:'tag-jade', REJECTED:'tag-cinnabar', WITHDRAWN:'tag-cinnabar' } as any)[s] ?? '' }
...
th              { text-align:left; font-weight:500; font-size:13px; color:var(--ink-muted); padding:12px 16px; white-space:nowrap; overflow:hidden; }
td              { padding:13px 14px; border-top:1px solid var(--line); vertical-align:middle; overflow:hidden; }
tr:hover td     { background:rgba(250,246,238,0.6); }
.cell-uid       { color:var(--ink-muted); font-size:13px; font-variant-numeric:tabular-nums; white-space:nowrap; }
.cell-date      { color:var(--ink-muted); font-size:12px; white-space:nowrap; text-align:center; }
...
.bio-text       { color:var(--ink); font-size:14px; line-height:1.5; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
...
.detail-loading { text-align:center; padding:40px; color:var(--ink-muted); }
.detail-summary { display:flex; align-items:center; gap:14px; margin-bottom:18px; }
.detail-avatar  { width:52px; height:52px; border-radius:12px; background:var(--cream); display:flex; align-items:center; justify-content:center; font-size:26px; flex-shrink:0; }
.detail-name    { font-size:17px; font-weight:700; color:var(--ink); margin-bottom:6px; }
.detail-sub     { display:flex; align-items:center; flex-wrap:wrap; gap:4px; font-size:13px; color:var(--ink-muted); }
.detail-info-row { display:flex; gap:24px; background:var(--cream); border-radius:10px; padding:12px 16px; margin-bottom:16px; }
.detail-info-item { display:flex; flex-direction:column; gap:3px; }
.detail-info-label { font-size:11px; color:var(--ink-muted); }
.detail-info-value { font-size:14px; color:var(--ink); font-weight:500; }
.detail-section { margin-bottom:16px; }
.detail-section-label { font-size:12px; color:var(--ink-muted); font-weight:500; margin-bottom:8px; text-transform:uppercase; letter-spacing:0.04em; }
.detail-bio { font-size:14px; color:var(--ink); line-height:1.7; background:var(--cream); border-radius:8px; padding:12px 14px; }

/* ── Attachment grid ── */
.attachment-grid { display:flex; flex-wrap:wrap; gap:12px; }
.att-item { width:148px; }
.att-label { font-size:11px; color:var(--ink-muted); margin-bottom:4px; }
.att-img   { width:148px; height:96px; object-fit:cover; border-radius:8px; border:1px solid var(--line); display:block; cursor:zoom-in; transition:opacity 0.15s, box-shadow 0.15s; }
.att-img:hover { opacity:0.88; box-shadow:0 2px 10px rgba(60,50,30,0.18); }
.att-name  { font-size:11px; color:var(--ink-muted); margin-top:4px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }

/* ── Lightbox ── */
.lightbox-mask { position:fixed; inset:0; background:rgba(0,0,0,0.82); z-index:2000; display:flex; align-items:center; justify-content:center; cursor:zoom-out; }
.lightbox-img  { max-width:90vw; max-height:90vh; border-radius:6px; box-shadow:0 8px 40px rgba(0,0,0,0.5); cursor:default; object-fit:contain; }
.lightbox-close { position:absolute; top:20px; right:24px; background:rgba(255,255,255,0.15); border:none; color:white; font-size:28px; cursor:pointer; border-radius:50%; width:40px; height:40px; display:flex; align-items:center; justify-content:center; transition:background 0.15s; }
.lightbox-close:hover { background:rgba(255,255,255,0.28); }

/* ── Toast ── */
.toast { position:fixed; bottom:32px; left:50%; transform:translateX(-50%); display:flex; align-items:center; gap:10px; padding:12px 20px; border-radius:10px; font-size:14px; font-family:inherit; box-shadow:0 4px 20px rgba(0,0,0,0.18); z-index:3000; min-width:260px; max-width:480px; }
.toast-success { background:#2d5a4f; color:white; }
.toast-error   { background:#8B2020; color:white; }
.toast-icon    { font-size:16px; font-weight:700; flex-shrink:0; }
.toast-msg     { flex:1; line-height:1.45; }
.toast-close   { background:transparent; border:none; color:inherit; font-size:18px; cursor:pointer; opacity:0.7; flex-shrink:0; padding:0; }
