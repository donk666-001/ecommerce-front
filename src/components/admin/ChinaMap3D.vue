    ...
    >
        <Transition name="tip-fade">
            ...
            >
                ...
                <span class="tip-val"
                    >{{ tip.value.toLocaleString("zh-CN") }} 人</span
                >
            </div>
        </Transition>
...
const geoJsonUrl = new URL("../../assets/maps/china.geojson", import.meta.url)
    .href;
...
const SPECIAL_REGION_MARKERS = [
    {
        ...
        label: "中国香港",
        coord: [114.18, 22.28],
        labelOffset: [0.34, 0.2],
    },
    {
        name: "澳门",
        label: "中国澳门",
        coord: [113.56, 22.16],
        labelOffset: [-0.34, -0.18],
    },
] as const;
...
function normalize(geoName: string): string {
    return geoName
        .replace(
            /(特别行政区|壮族|回族|维吾尔|藏族|苗族|彝族|自治区|省|市)/g,
            "",
        )
        .slice(0, 4);
}
...
function findStat(geoName: string): Info | null {
    ...
    const d = props.data.find(
        (p) => geoName.includes(p.name) || p.name.includes(short),
    );
    if (!d) return null;
...
function buildShape(ring: number[][]): THREE.Shape | null {
    ...
    const pts = ring.map(
        (point) => new THREE.Vector2(...proj(point[0]!, point[1]!)),
    );
...
function shouldRenderPolygon(geoName: string, outerRing: number[][]) {
    ...
    if (geoName.includes("海南") && bounds.maxLat < HAINAN_OFFSHORE_MAX_LAT)
        return false;
    if (
        geoName.includes("广东") &&
        bounds.maxLat <= GUANGDONG_SOUTH_ISLAND_MAX_LAT
    )
        return false;
    return true;
}
        ...
        for (const poly of polys) {
            ...
            if (stat) {
                const info: Info = {
                    name: stat.name,
                    value: stat.value,
                    mat: mesh.material as THREE.MeshLambertMaterial,
                    baseZ: 0,
                    activeZ: 0.07,
                };
...
function onMouseMove(e: MouseEvent) {
    ...
    raycaster.setFromCamera(pointer, camera);
    const hit = raycaster.intersectObjects(hitMeshes)[0]?.object as
        | THREE.Mesh
        | undefined;
