    ...
    <div class="perms-panel">
        ...
        <Teleport to="body">
            ...
            >
                <button
                    ...
                    :class="{
                        'ap-role-selected': currentOpenUser.roleCode === code,
                    }"
                    @click="selectRole(currentOpenUser, code)"
                >
                    <span
                        class="ap-role-dot"
                        :style="{ background: ROLE_COLORS[code] }"
                    ></span>
                    <span class="ap-role-opt-label">{{ label }}</span>
                    <svg
                        v-if="currentOpenUser.roleCode === code"
                        width="12"
                        height="12"
                        viewBox="0 0 24 24"
                        fill="none"
        ...
        <div class="section-header">
            <div class="section-title">
                <span class="section-dot"></span>权限设置
            </div>
        </div>
                        ...
                        >
                            <td>
                                ...
                                >
                                    ...
                                    <span v-if="isSelf(u)" class="self-badge"
                                        >本账号</span
                                    >
                                </div>
                            </td>
                                ...
                                <div class="ap-role-select" @click.stop>
                                    ...
                                    >
                                        <span
                                            ...
                                            :style="{
                                                background:
                                                    ROLE_COLORS[u.roleCode],
                                            }"
                                        ></span>
                                        <span class="ap-role-name">{{
                                            ROLE_LABELS[u.roleCode] ?? "未知"
                                        }}</span>
                                        <svg
                                            class="ap-chevron"
                                            width="10"
...
const ROLE_ENTRIES: [number, string][] = [
    [100, "管理员"],
    [200, "专家"],
    [300, "普通用户"],
];
...
function selectRole(u: UserAdminVO, newRoleCode: number) {
    ...
    showConfirm({
        ...
        onConfirm: async () => {
            ...
            showToast(
                `已将 "${u.nickname}" 的角色修改为【${ROLE_LABELS[newRoleCode]}】`,
            );
        },
    });
}
...
.ap-role-trigger.ap-open .ap-chevron {
    transform: rotate(180deg);
}
...
.ap-role-menu {
    ...
    border-radius: 10px;
    box-shadow:
        0 4px 24px rgba(40, 30, 20, 0.13),
        0 1px 4px rgba(40, 30, 20, 0.06);
    overflow: hidden;
...
.ap-role-option.ap-role-selected {
    font-weight: 600;
}
.ap-role-option .ap-role-dot {
    width: 7px;
    height: 7px;
    border-radius: 50%;
    flex-shrink: 0;
}
.ap-role-option .ap-role-opt-label {
    flex: 1;
}
