          ...
          <aside class="geo-rank-pane">
            ...
            <div class="geo-rank-list">
              <button
                ...
                type="button"
                class="geo-rank-item"
                :class="{ active: activeProvinceName === item.name }"
              ...
              >
                <div class="geo-rank-label">
                  <span class="geo-rank-index">{{ String(index + 1).padStart(2, '0') }}</span>
                  <span class="geo-rank-name">{{ item.name }}</span>
                </div>
                <div class="geo-rank-bar-track">
                  <div class="geo-rank-bar-fill" :style="{ width: `${(item.value / topProvinceMax) * 100}%` }"></div>
                </div>
                <div class="geo-rank-value">{{ item.value }}人</div>
              </button>
            </div>
          </aside>
        </div>
...
// ── KPI data ───────────────────────────────────────────────────────────────
const kpiRow1 = ref([
  { id: 'users',   label: '平台总用户数',     display: '…',      color: 'jade',    icon: '👥', trend: '↑ +12 今日新增',   trendDir: 'up' },
...
const kpiRow2 = ref([
  { id: 'experts',  label: '专家入驻总数',    display: '…',     color: 'jade',     icon: '👨‍⚕️', trend: '加载中…',        trendDir: 'up',   sub: '',             alert: false },
  { id: 'pending',  label: '待审核专家',      display: '…',     color: 'cinnabar', icon: '📝', trend: '加载中…',        trendDir: 'warn', sub: '',             alert: false },
  { id: 'content',  label: '内容发布（本月）', display: '3,420', color: 'jade',     icon: '📚', trend: '↑ +48 本周',    trendDir: 'up',   sub: '节气专题 12 个', alert: false },
  { id: 'newusers', label: '今日新增用户',    display: '…',     color: 'gold',     icon: '✨', trend: '加载中…',       trendDir: 'up',   sub: '',            alert: false },
])
...
const topProvinceStats = computed(() => [...provinceStats].sort((a, b) => b.value - a.value).slice(0, 12))
const topProvinceMax = computed(() => topProvinceStats.value[0]?.value ?? 1)
const selectedProvinceName = ref('')
const hoverProvinceName = ref('')
const activeProvinceName = computed(() => hoverProvinceName.value || selectedProvinceName.value)
    ...
    c.setOption({
      baseOption: {
        ...
        legend: {
          ...
          itemHeight: 11,
          itemGap: 16,
          textStyle: { color: '#596D68', fontSize: 14, lineHeight: 18 },
        },
        series: [{
          name: '节气专题',
          type: 'pie',
          ...
          emphasis: {
            scaleSize: 8,
            label: { show: true, fontSize: 15, fontWeight: 600, color: '#2C3639', formatter: '{b}\n{c}次' },
          },
          data: solarData.map(d => ({ name: d.name, value: d.value, itemStyle: { color: d.color, borderWidth: 3, borderColor: '#fff' } })),
        }],
      },
      media: [
        {
          query: { maxWidth: 520 },
          option: {
            legend: { orient: 'horizontal', left: 'center', right: 'auto', bottom: 2, itemGap: 12, textStyle: { fontSize: 12, lineHeight: 16 } },
            series: [{ radius: ['42%', '68%'], center: ['50%', '42%'] }],
          },
        },
        {
          query: { minWidth: 521, maxWidth: 660 },
          option: {
            legend: { right: '3%', itemGap: 12, textStyle: { fontSize: 13, lineHeight: 17 } },
            series: [{ radius: ['45%', '73%'], center: ['33%', '52%'] }],
          },
        },
      ],
    })
...
function initCounters() {
  ;([
    ['kpi-consult', 1256], ['kpi-ai', 3842], ['kpi-content', 3420],
...
onMounted(async () => {
  ...
  await Promise.allSettled([
    ApiAdmin.getUserStats().then(res => {
      const raw = (res as any)?.data?.data as { total: number; todayNew: number; yesterdayNew: number }
      ...
      usersCard.trend    = todayNew > 0 ? `↑ +${todayNew} 今日新增` : '今日暂无新增'
    ...
    ApiExpert.adminGetStats().then(res => {
      const raw = (res as any)?.data?.data as { total: number; todayNew: number }
      const total    = raw?.total    ?? 0
      const todayNew = raw?.todayNew ?? 0
      const card = kpiRow2.value.find(c => c.id === 'experts')!
      card.display  = String(total)
      card.trend    = todayNew > 0 ? `↑ +${todayNew} 今日新增` : '今日暂无新增'
...
.geo-chart-body  { padding: 14px 18px 18px; }
.geo-layout {
  display: grid;
...
.geo-map-pane {
  ...
  border-radius: 12px;
  background:
    radial-gradient(circle at 18% 22%, rgba(201,165,92,0.08), transparent 34%),
    linear-gradient(180deg, rgba(255,255,255,0.96) 0%, rgba(250,246,238,0.82) 100%);
}
.geo-chart       { height: 440px; width: 100%; }
.geo-rank-pane {
  border: 1px solid rgba(232,223,208,0.58);
  border-radius: 12px;
  background: linear-gradient(180deg, rgba(255,255,255,0.96) 0%, rgba(250,246,238,0.82) 100%);
...
.dashboard-bottom-row .status-strip { order: 1; }
.dashboard-bottom-row .chart-card { order: 2; }
...
@media (max-width: 1024px) {
  ...
  .chart-row-2-1, .chart-row-1-2, .dashboard-bottom-row { grid-template-columns: 1fr; }
  .geo-layout { grid-template-columns: 1fr; }
  .geo-map-pane,
  .geo-rank-pane,
  .geo-chart { min-height: 0; height: 360px; }
  .geo-rank-pane { height: auto; }
  .solar-chart { height: 300px; }
  .dashboard-bottom-row .status-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); }
  .dashboard-bottom-row .status-item:nth-child(2),
